...
export function SecurityWorkspace({ centerName, email, initialEnabled, requiredForPlatform }: {
  ...
  async function confirmSetup(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (!response.ok) throw new Error(response.status === 410 ? "انتهت مهلة الإعداد. ابدأ من جديد." : await responseMessage(response));
  ...
  async function disable(event: FormEvent<HTMLFormElement>) {
    ...
    if (!window.confirm("إيقاف التحقق بخطوتين لحسابك في المراكز؟")) return;
        ...
        {enabled ? <>
          ...
          {requiredForPlatform ? <p className="muted">هذا الحساب يدخل لوحة المنصة، لذلك يبقى التحقق مطلوبًا لها ولا يمكن إيقافه من المركز.</p> :
            <form className="form-stack" onSubmit={disable}>
              <FormField id="disable-password" label="كلمة المرور الحالية" type="password" value={disablePassword} onChange={setDisablePassword} autoComplete="current-password" required />
              {useRecovery ? <FormField id="disable-recovery" label="رمز الاستعادة" value={recoveryCode} onChange={setRecoveryCode} direction="ltr" required /> :
                <FormField id="disable-code" label="رمز المصادقة الحالي" value={code} onChange={setCode} autoComplete="one-time-code" direction="ltr" required />}
          ...
          <form className="form-stack" onSubmit={confirmSetup}>
            <FormField id="setup-code" label="رمز التحقق" value={code} onChange={setCode} autoComplete="one-time-code" direction="ltr" required />
          ...
          <form className="form-stack" onSubmit={startSetup}>
            <FormField id="setup-password" label="كلمة المرور الحالية" type="password" value={setupPassword} onChange={setSetupPassword} autoComplete="current-password" required />
